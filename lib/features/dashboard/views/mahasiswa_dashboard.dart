...
import '../../ujian/views/join_ujian_view.dart';
import '../../ujian/views/riwayat_mahasiswa_view.dart';
import '../../ujian/views/waiting_room_view.dart';
...
class DashboardMahasiswaView extends StatefulWidget {
  const DashboardMahasiswaView({super.key});

  @override
  State<DashboardMahasiswaView> createState() =>
      _DashboardMahasiswaViewState();
}

class _DashboardMahasiswaViewState extends State<DashboardMahasiswaView> {
  int _currentIndex = 1;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      context.read<MahasiswaUjianController>().checkOfflineSubmission(mahasiswaId: mahasiswaId);
    });
  }
...
class BerandaContent extends StatelessWidget {
      ...
      children: [
        ...
        Expanded(
          child: RefreshIndicator(
            onRefresh: () async {
              final authVm = context.read<AuthController>();
              await context.read<MahasiswaUjianController>().checkOfflineSubmission(mahasiswaId: authVm.mahasiswaId);
            },
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const JoinUjianView(),
                ),
              ),
                ...
                child: IconButton(
                  ...
                  onPressed: () async {
                    ...
                    if (confirm == true && context.mounted) {
                      ...
                      if (context.mounted) {
                        final authVm = context.read<AuthController>();
                        context.read<MahasiswaUjianController>().checkOfflineSubmission(mahasiswaId: authVm.mahasiswaId);
  ...
  Widget _buildTungguSinkronCard(BuildContext context, MahasiswaUjianController vm) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
