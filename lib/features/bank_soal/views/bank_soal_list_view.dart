...
import 'buat_bank_soal_view.dart';
...
/// This is separate from ${1}View (publish-only view).
class BankSoalListView extends StatefulWidget {
  const BankSoalListView({super.key});

  @override
  State<BankSoalListView> createState() => _BankSoalListViewState();
}

class _BankSoalListViewState extends State<BankSoalListView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final dosenId = context.read<AuthController>().userData?['id'];
      if (dosenId != null) {
  ...
  Future<void> _refresh() async {
    final dosenId = context.read<AuthController>().userData?['id'];
    if (dosenId != null) {
      await context.read<DosenUjianController>().fetchUjianForDosen(
        dosenId as int,
      );
    }
  }
  ...
  Future<void> _openCreatePage({
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => BuatBankSoalView(
